/*
TCSS    
Julio Buonfigli    //para mejorar: que trabaje sobre imagenes duplicadas al principio y no modifique las abiertas
*/
...
function dispersar(vec1, w, h, Dai, image, name) 
{
vec2=newArray(w*h);
res=newArray(w*h);
j=0;
...
function contarCeros(ventana)       //sobre ventana!!!!!               
{
...
function contarUnos(ventana)         //sobre ventana!!!!!!            
{
selectWindow(ventana);       
   a = newArray(w*h);
   i = 0;
...
{
dc=newArray(7);
dc[0]=dc[1]=dc[2]=dc[3]=dc[4]=dc[5]=dc[6]=0;
for(i=0; i<unos; i++)                                                                                               
	{
	if(v1[i]*v2[i]*v[3] != 0)
		{
...
for(i=id1-1000; i<id1+1000; i++)
  ...
  Dialog.addNumber("Average intensity (16-bit image): ", 6000);
...
run("Size...", "width=niw height=nih interpolation=None");}
if(mascara!="none") {selectWindow("Mask");
run("Size...", "width=niw height=nih interpolation=None");}
}
...
if(DS==true)
{
dispersar(R, w, h, Dai, "Red", "Red");   
dispersar(V, w, h, Dai, "Green", "Green");
if(azul!="none") { dispersar(A, w, h, Dai, "Blue", "Blue");}
}
...
if(intOR!="No_threshold")    //genera la mascara                          
{
	...
	if(operacion=="OR")
		{
		imageCalculator("OR create", "rojoB", "verdeB"); 
	        rename("mascaraF");
        	if(azul!="none") {imageCalculator("OR create", "mascaraF", "azulB"); rename("mascaraF");}
		}
		else
		{
		imageCalculator("AND create", "rojoB", "verdeB"); 
	        rename("mascaraF");
        	if(azul!="none") {imageCalculator("AND create", "mascaraF", "azulB"); rename("mascaraF");}
		}
	selectWindow("mascaraF");
	setThreshold(1, 255);
	run("Convert to Mask");
	if(mascara!="none") { imageCalculator("AND create", "mascaraF", "Mask");}
	rename("Mask");
	}
                      
if(intOR=="No_threshold" && mascara=="none")    //vectorizacion de la mascara
{
unosMascara=w*h+1;
cerosMascara=0;
mask=newArray(w*h); 
...
{
MUR=ThresholdValue("Red", intOR);
MUV=ThresholdValue("Green", intOR);
if(azul!="none"){MUA=ThresholdValue("Blue", intOR); } 
}
...
for(i=0; i<unosMascara; i++)                                                                                                
	...
	if(azul!="none") {denA=denA+(az[i]-pa)*(az[i]-pa);}
	if(azul!="none") {numVA=numVA+(v[i]-pv)*(az[i]-pa);}
	if(azul!="none") {numRA=numRA+(r[i]-pr)*(az[i]-pa);}
...
r=newArray(unosMascara); r=vectorizar(rojo, w, h, MUR, mask, unosMascara);
v=newArray(unosMascara); v=vectorizar(verde, w, h, MUV, mask, unosMascara);
if(azul!="none") { a=newArray(unosMascara); a=vectorizar(azul, w, h, MUA, mask, unosMascara); }

sumRojo=sumVerde=unosRojo=unosVerde=sumAzul=unosAzul=0;
for(i=0; i<unosMascara; i++){
	sumRojo=sumRojo+r[i];                    
	sumVerde=sumVerde+v[i];
	if(azul!="none") {sumAzul=sumAzul+a[i];}
	if(r[i]>0)
		unosRojo++;
	if(v[i]>0)
		unosVerde++; 
...
if(isOpen("Mask")) {cerrarMask=false} else { newImage("Mask", "8-bit black", w, h, 1);  run("Invert"); cerrarMask=true;}

selectWindow(rojo);  run("Duplicate...", "title=rojoB");          
setThreshold(MUR, 255);
run("Convert to Mask");
imageCalculator("AND create", "rojoB", "Mask");   
rename("rojoAndMascara");
selectWindow(verde);  run("Duplicate...", "title=verdeB");          
setThreshold(MUV, 255);
run("Convert to Mask");
imageCalculator("AND create", "verdeB", "Mask");  
rename("verdeAndMascara");
if(azul!="none") {selectWindow(azul);  run("Duplicate...", "title=azulB");          
...
if(cerrarMask==true)  {selectWindow("Mask"); close();}
if(azul!="none") {selectWindow("azulB"); close();}
