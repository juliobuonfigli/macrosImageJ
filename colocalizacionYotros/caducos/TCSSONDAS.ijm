...
for(Y=0; Y<h; Y++)
	...
	for(X=0; X<w; X++)
		...
		for(y=0; y<h; y++)
			...
			for(x=0; x<w; x++)
				{
				if(i==j) {} else {  //ver como se comporta el dispersar según distintas imágenes
				vec2[i]=vec2[i]+round(vec1[j]/sqrt((X-x)*(X-x)+(Y-y)*(Y-y)));}
				//vec2[i]=vec2[i]+round(vec1[j]/((X-x)*(X-x)+(Y-y)*(Y-y)));}
				i++;
				}
			}
		j++;	
...
{
Dai=6000;
dispersar(R, w, h, Dai, "Red", "Red");   selectWindow("Red"); run("8-bit");
dispersar(V, w, h, Dai, "Green", "Green"); selectWindow("Green"); run("8-bit");
if(azul!="none") { dispersar(A, w, h, Dai, "Blue", "Blue");  selectWindow("Blue"); run("8-bit");}
}
...
run("Convert to Mask");
imageCalculator("AND create", "Blue", "MASK");                       
