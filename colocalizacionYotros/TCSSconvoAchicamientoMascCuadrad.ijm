...
function recortar(vec, ancho, alto, r)                       
{
  W=ancho-2*r;
  H=alto-2*r;
  ar=newArray(W*H);
 ...
 for(y=0; y<alto; y++)
	...
	for(x=0; x<ancho; x++)
		{
		if(x>ancho-r-1 || x<r || y<r || y>alto-r-1) {i++;}
		else {
		ar[j]=vec[i];  j++; i++;}
		//i++;	
		}
...
for(i=id1-1000; i<id1+1000; i++)
  ...
  Dialog.addChoice("Dispersing function: ", newArray("Aritmetica", "Lineal", "Cuadratica", "Cubica", "Geometrica"));
  Dialog.addNumber("Radius: ", 8);
...
r=recortar(R, w, h, ra);
v=recortar(V, w, h, ra);
if(azul!="none") a=recortar(A, w, h, ra); 
w=w-2*ra; h=h-2*ra; unosMascara=w*h; 
...
r=umbralizar(r, MUR, w*h); R=umbralizar(R, MUR, w*h); 
v=umbralizar(v, MUV, w*h); V=umbralizar(V, MUV, w*h); 
if(azul!="none") { a=umbralizar(a, MUA, w*h); A=umbralizar(A, MUA, w*h);  }
/*selectWindow("Red");
...
doblesRV=dobleColo(rs, v, unosMascara, sumRojo, sumVerde, unosRojo, unosVerde);
if(azul!="none") {vs=randomizar(v, unosMascara); doblesRA=dobleColo(rs, a, unosMascara, sumRojo, sumAzul, unosRojo, unosAzul);
doblesAV=dobleColo(a, vs, unosMascara, sumAzul, sumVerde, unosAzul, unosVerde);}
...
if(rvvr==vaav && rvvr==arra)
	...
	{
	if(rvvr>=vaav && rvvr>=arra)                               
		{vs=v; rs=r; as=randomizar(a, unosMascara); }
...
if(azul!="none") triples=tripleColo(rs, vs, as, unosMascara, sumRojo, sumVerde, sumAzul, unosRojo, unosVerde, unosAzul);
...
if(azul!="none") {
	run("Merge Channels...", "c1=[rojoB] c2=[verdeB] c3=[azulB] create");    
	run("Canvas Size...", "width=w height=h position=Center zero"); }
	else {
	run("Merge Channels...", "c1=[rojoB] c2=[verdeB] create");
	run("Canvas Size...", "width=w height=h position=Center zero"); }
