...
frag=25; vd=10; sd=true; MO=false;                        //basic
...
eq=true; radiusUM=radiusUM; radiusM=radiusM; subt=40; NORM=true; EMO=true; CC=true; RC=10;                     //treatment   
EIV=false; sep1=6; sep2=3; esp1=12; esp2=18; ra=2; pol=true;                                                  //sampling
MAX=40; MEAN=15; ESE=true; ESO=false; EAE=true; EAO=false;                                                   //evaluation
BM=false;

/*if(MO==true) {
Dialog.create("Fusion-Fision");                              //secundary dialog                            
  Dialog.addMessage("STACK TREATMENT");                      //deprecated    
  ...
  Dialog.addNumber("Minimal mask ratio: ",  RC);
  Dialog.show(); 
  ...
  Dialog.addNumber("Separation between Main and inner ROI (pixels):", sep1);     
...
//  Dialog.addCheckbox("Polygonal outer ROI", pol);  //deprecated 
  //Dialog.show(); 
  
//  if(MO==true) {
  //Dialog.create("Fusion-Fision");      
  Dialog.addMessage("EVALUATION"); 
  //Dialog.addNumber("Event maximum signal threshold: ", MAX);     //deprecated 
  //Dialog.addNumber("Event mean signal threshold: ", MEAN);       //deprecated 
...
dt=pi/frag;  cont2=0;
X=newArray(50); Y=newArray(50); per=newArray(frames); area=newArray(frames);
selectWindow("mainROIs");
...
for(j=1; j<frames+1; j++)    //main ROIs
	...
	roiManager("Rename", j);
	run("Measure"); cont2++;
	per[j-1]=getResult("Preim.", 0);
	area[j-1]=getResult("Area", 0);
	run("Clear Results");
	}
...
for(j=0; j<frag; j++)
	...
	for(i=1; i<frames; i++)
		...
		run("Select None");
		xa=xp[j]+sep1*cos(t+2*dt);  ya=yp[j]+sep1*sin(t+2*dt);
		XB=xp[j]+(esp1+sep1)*cos(t+2*dt); YB=yp[j]+(esp1+sep1)*sin(t+2*dt);
		xb=xp[j+1]+sep1*cos(t+4*dt); yb=yp[j+1]+sep1*sin(t+4*dt);	
		XA=xp[j+1]+(esp1+sep1)*cos(t+4*dt); YA=yp[j+1]+(esp1+sep1)*sin(t+4*dt);
		if(j==frag-1) {
		xb=xp[0]+sep1*cos(t+4*dt); yb=yp[0]+sep1*sin(t+4*dt);	
		XA=xp[0]+(esp1+sep1)*cos(t+4*dt); YA=yp[0]+(esp1+sep1)*sin(t+4*dt); }
		makePolygon(xa, ya, xb, yb, XA, YA, XB, YB);
		run("Measure");
		roiManager("Add");
		roiManager("Select", cont2);
		roiManager("Rename", i+"-"+j+1+"-"+1);
		v1[cont]=getResult("Mean", 0);
		m1[cont]=getResult("Max", 0);
		run("Clear Results"); 	cont2++;
		if(sd==true) wait(10);
		run("Select None");
		xa=xp[j]+(esp1+sep1+sep2)*cos(t); ya=yp[j]+(esp1+sep1+sep2)*sin(t);
		XB=xp[j]+(esp1+sep1+sep2+esp2)*cos(t); YB=yp[j]+(esp1+sep1+sep2+esp2)*sin(t);
		xb=xp[j+1]+(esp1+sep1+sep2)*cos(t+6*dt); yb=yp[j+1]+(esp1+sep1+sep2)*sin(t+6*dt);	
		XA=xp[j+1]+(esp1+sep1+sep2+esp2)*cos(t+6*dt); YA=yp[j+1]+(esp1+sep1+sep2+esp2)*sin(t+6*dt);
		if(j==frag-1) {
		xb=xp[0]+(esp1+sep1+sep2)*cos(t+6*dt); yb=yp[0]+(esp1+sep1+sep2)*sin(t+6*dt);	
		XA=xp[0]+(esp1+sep1+sep2+esp2)*cos(t+6*dt); YA=yp[0]+(esp1+sep1+sep2+esp2)*sin(t+6*dt); }
		makePolygon(xa, ya, xb, yb, XA, YA, XB, YB);
		run("Measure");
		roiManager("Add");
		roiManager("Select", cont2);
		roiManager("Rename", i+"-"+j+1+"-"+2);
...
Plot.setLimits(0, frames, 0, 10000);  
