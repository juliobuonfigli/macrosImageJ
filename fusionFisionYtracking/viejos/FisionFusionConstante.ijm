...
fA2=1.21;                  //separacion entre anillos internos y externo
...
umbralRazon=0.25;            //razón entre entre anillo interno y externo (aceptación negra)
//umbralPR=2;            //multiplica el promedio general (aceptacion negra)  
umbralDVA=0.7;           //multiplica el promedio (aceptacion verde). 
umbralDRA=1.6;              //multiplica el promedio (aceptacion rojo)
...
run("Bandpass Filter...", "filter_large=10000 filter_small=11 suppress=None tolerance=5 process");
run("Unsharp Mask...", "radius=4 mask=0.97 stack");
...
for(k=0; k<frames-1; k++)
	...
	if(e[k]>umbralDRA*pe && q[k]>umbralRazon) 	
	 	{
		EVENTO++;
