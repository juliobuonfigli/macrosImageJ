...
frag=30;                      //fragmentación de anillos. El numero de puntos de analisis es la mitad de este numero
dt=2*pi/frag;                //diferencial tiempo
fA=1.1;                     //espesor de anillo
fA2=1.16;                  //separacion entre anillos internos y externo
...
umbralDVA=0.6;           //multiplica la desviacion estandar (aceptacion verde)
umbralDRA=1.6;              //multiplica la desviacion estandar (aceptacion rojo)
umbralDVR=0.8;              //multiplica la desviacion estandar (rechazo verde)
t=0;
rename("Final");
/*
...
run("Unsharp Mask...", "radius=3 mask=0.92 stack");
...
run("Close");
run("ROI Manager...");
evento=newArray(frag/2+1);
fision=newArray(frag/2+1);
fusion=newArray(frag/2+1);
noCal=newArray(frag/2+1);
fuerza=newArray(frag/2+1);
intensidad=newArray(frag/2+1);
longitud=newArray(frag/2+1);
...
for(i=1; i<frag/2+1; i++)  
	...
	for(k=0; k<frames-1; k++)
		promMem=promMem+getResult("Mean", k)/255;	             
	s=frames-1;	
	for(k=0; k<frames-1; k++)
		{
		promExt=promExt+getResult("Mean", s)/255;	
		s++;
		} 
	t=t+2*dt;
	}
promMem=promMem/(frag*frames);		
...
for(k=0; k<frames-1; k++)
	{
	if(e[k]>umbralDRA*promExt)     //aca la condición no será en función de la desviacion estandar sino en funcion a un factor multiplicador (1,3) 
		{
		EVENTO++;
		INTENSIDAD=INTENSIDAD+q[k];
		if(w[k]<umbralDVA*promMem)
			event[k]=1;
			else
			{
			if(w[k]>umbralDVA*promMem && w[k]<umbralDVR*promMem)
				event[k]=2;
				else
				event[k]=3;
			}		
...
fuerza[i]=FUERZA;
	 ...
	 for(f=0; f<61; f++)     //creacion del plot
	 	{
	 	a[f]=f+1;
	 	promMA[f]=umbralDVA*promMem;
		promE[f]=umbralDRA*promExt;
	 	promMR[f]=umbralDVR*promMem;
	 	}
	 n = q.length;
...
run("Images to Stack", "name=Stack title=30 use");
...
print(f, "\\Headings:Cuadrante\tEventos\tFusiones\tFisiones\tNo calificados\tIntensidad prom.\tIntensidad total\tDuración prom.");
for(i=1; i<frag/2+1; i++)
	{
	print(f, i+"\t  "+evento[i]+"\t  "+fusion[i]+"\t  "+fision[i]+"\t  "+noCal[i]+"\t  "+fuerza[i]+"\t  "+intensidad[i]+"\t  "+longitud[i]);	
	}
print(f, "");
print(f, " Totales"+"\t   "+EVENTO+"\t  "+FUSION+"\t  "+FISION+"\t  "+NOCAL+"\t  "+FUERZA+"\t  "+INTENSIDAD+"\t  "+LONGITUD);
