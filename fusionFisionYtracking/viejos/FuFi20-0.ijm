...
for(i=1; i<frag+1; i++)    
	...
	for(j=1; j<frames+1; j++)   
		...
		poligono(sqrt(esp), esp, cx, cy, t-(ra-1)*dt, ra*dt, ow, oh, j, false, DX, DY);
		run("Measure");
		if(j==1)
			roiManager("Add");
		v3[cont]=getResult("Mean", 0)/255;
		run("Clear Results"); wait(sd); run("Select None");
		poligono(sqrt(esp), esp*sqrt(esp), cx, cy, t-(ra-1)*dt, ra*dt, ow, oh, j, false, DX, DY);
		run("Measure");
		if(j==1)
			roiManager("Add");
		v4[cont]=getResult("Mean", 0)/255;
		run("Clear Results"); wait(sd);  run("Select None");
		poligono(sqrt(esp), esp*esp, cx, cy, t-(ra-1)*dt, ra*dt, ow, oh, j, false, DX, DY);
		run("Measure");
		if(j==1)
			roiManager("Add");
...
e1=newArray(frames*frag-frag); e2=newArray(frames*frag-frag); e3=newArray(frames*frag-frag); e4=newArray(frames*frag-frag); 
e5=newArray(frames*frag-frag); e6=newArray(frames*frag-frag); e7=newArray(frames*frag-frag); 
pv1=newArray(frag); pv2=newArray(frag); pv3=newArray(frag); pv4=newArray(frag); 
sal=newArray(frag*frames); UG=newArray(frag*frames-frag);
ev=newArray(frames*frag-frag); for(i=0; i<frag*frames-frag; i++) {ev[i]=false; sal[i]=0;}
...
T=0;
v1de=0; v1p=0;
for(e=0; e<frag*frames-frag; e++) {
	if(e%(frames-1)==0) {} else
	v1p=v1p+log(v1[e]/v1[e+1]);
v1p=v1p/(frag*frames);	
for(e=0; e<frag*frames-frag; e++)
	if(e%(frames-1)==0) {} else
	v1de=v1de+pow(log(v1[e]/v1[e+1])-v1p, 2);
v1de=sqrt(v1de/(frag*frames));	
